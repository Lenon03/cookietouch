...
exports.getPath = function(source, target, occupiedCells, allowDiagonals, stopNextToTarget) {
  var c,
    candidate;
  console.log("[pathfinding]Calculatig path for " + source + " to " + target);

  allowDiagonals = allowDiagonals === undefined
    ? true
    : !!allowDiagonals;
  ...
  if (srcCell.zone === -1) {
    ...
    if (bestFit !== null) {
      console.log("[pathfinding]Selected : " + getCellId(bestFit.i - 1, bestFit.j - 1));
      return [
        source,
        getCellId(bestFit.i - 1, bestFit.j - 1)
      ];
    }
  ...
  console.log("[pathfinding]Select : " + selection + ", dist : " + distSrcDst);
  ...
  console.log(exports.logPath(shortestPath));
  return shortestPath;
};
...
function checkPath(path) {
  ...
  for (var i = 1, len = path.length; i < len; i += 1) {
    var cellId = path[i];
    var coord = getMapPoint(cellId);
    if (Math.abs(previous.x - coord.x) > 1) {
      return false;
    }
    if (Math.abs(previous.y - coord.y) > 1) {
      return false;
    }
    previous = coord;
  }
...
//▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄
/* sa c´est moi qui l´est fait :P (enfin presque mdr) */
// exports.getCellsWithDirection = function(path){
// 	var compressedPath = {};
...
// 		}
//
// 		compressedPath[cellId] = direction;
//
//
// 		prevCellId = cellId;
// 		prevX = coord.x;
