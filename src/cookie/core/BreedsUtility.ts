...
export default class BreedsUtility {
  ...
  public static getBreedHeads(breedId: number, sex: number): List<string> {
    return Enumerable.Range(1, 8).Select((o) => `${DTConstants.config.assetsUrl}/gfx/cosmetics/${breedId}${sex}_${o}.png`);
  }

  public static getCosmeticId(breedId: number, sex: boolean, headOrder: number): number {
  ...
  public static getBreedBaseColors(breed: Breeds, sex: number): Color[] {
    return (sex === 0 ? new List(breed.maleColors).Select(this.parseIndexedColor)
      : new List(breed.femaleColors).Select(this.parseIndexedColor)).ToArray();
  }
