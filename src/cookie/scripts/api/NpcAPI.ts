...
export default class NpcAPI {
  ...
  public npcBank(npcId: number, replyId: number): boolean {
    ...
    this.account.scripts.actionsManager.enqueueAction(new NpcBankAction(npcId, replyId), true);
    return true;
  }
  ...
  public npc(npcId: number, actionIndex: number): boolean {
    ...
    this.account.scripts.actionsManager.enqueueAction(new NpcAction(npcId, actionIndex), true);
    return true;
  }

  public reply(replyId: number) {
    this.account.scripts.actionsManager.enqueueAction(new ReplyAction(replyId), true);
  }
}
