...
export default class FileManipulator extends Task {
  ...
  public enableAdminMode() {
    this.scriptContent =
      this.scriptContent.replace(/([a-zA-Z]{1,2}\.prototype)\.isModeratorOrMore.[\S\s]*\1(.isModerator)/,
        "$1.isModeratorOrMore=function(){return true},$1$2");
  }

  public displaySentMessages() {
    this.scriptContent =
      this.scriptContent.replace(/([a-zA-Z]{1,2})\.sendMessage=function\(([a-zA-Z]{1,2}),([a-zA-Z]{1,2})\){\1\.send\("sendMessage"/,
        '$1.sendMessage=function($2,$3){top.console.log($2,$3);$1.send("sendMessage"');
  }

  public disableAppBackground() {
  ...
  public enableAccessToAllFunctions() {
    this.scriptContent =
      this.scriptContent
        .replace(/(return ([a-zA-Z]{1,2})\.([a-zA-Z]{1,2})=([a-zA-Z]{1,2}),\2\.([a-zA-Z]{1,2})=([a-zA-Z]{1,2}),\2\.([a-zA-Z]{1,2})="",\2\(0\))/,
        "window.master=$2;$1");
  }
  ...
  public disableChangeMapAnimations() {
    // rewrite loading transitions
    this.scriptContent =
      this.scriptContent.replace(/([a-zA-Z]{1,2}\.prototype)\._runLoadingFadeInTransition.[\S\s]*\1(._runLoadingFadeOutTransition)/,
        "$1._runLoadingFadeInTransition=function(){this._loadingFadeInTransitionRunning=!0,this._waitingForMessage=!0,this.showLoadingProgress(0);this.mapScene.renderingParams.ratio=0;var e=this;e._saveImageForTransition(\"current\"),e._loadingFadeInTransitionRunning=!1,e._startLoading();},$1$2");
    this.scriptContent =
      this.scriptContent.replace(/([a-zA-Z]{1,2}\.prototype)\._runLoadingFadeOutTransition.[\S\s]*\1(.makeActorWalkIn)/,
        "$1._runLoadingFadeOutTransition=function(){var e = this,t = this.mapScene;null !== e._mapSceneTransitionGraphic && (e._mapSceneTransitionGraphic.remove(), e._mapSceneTransitionGraphic = null);t.setShader(\"unfiltering\"), t.renderingParams.ratio = .15, e._hideLoadingProgress();},$1$2");
  }
  ...
  public createUtils() {
    ...
    // // losDetector
    // this.scriptContent =
    //   this.scriptContent.replace(/(([a-zA-Z]{1}).exports.getCellDistance=[a-zA-Z]{1},[a-zA-Z]{1}.exports.getDistance=[a-zA-Z]{1})/,
    //     "$1;window.utils.losDetector=$2.exports;");
    // // mapPoint
    // this.scriptContent =
    //   this.scriptContent.replace(/(([a-zA-Z]{1}).getOrientation=[a-zA-Z]{1},[a-zA-Z]{1}.getDistance=[a-zA-Z]{1})/, "$1;window.utils.mapPoint=$2;");
    // // atouin
    ...
    // // inactivityManager
    // this.scriptContent =
    //   this.scriptContent.replace(/([a-zA-Z]{1}.prototype._leaveInactiveMode=function(.*?))(\},function)/,
    //     "$1;window.utils.inactivityManager=e.exports;$3");
  ...
  public do(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      // replace(/(var.*=.*\.touches\s\|\|.*\[\],)/g, 'if (e.type === "mousedown" || e.type === "mouseup") {
      // return o.x = e.clientX, o.y = e.clientY, { x: o.x, y: o.y, touchCount: "mouseup" === e.type ? 0 : 1, touches: [{x: o.x, y: o.y }] } }\n$1')
    });
  }
